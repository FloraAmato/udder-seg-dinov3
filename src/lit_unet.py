...
from .unet_baseline import get_unet_baseline


class LitUNetModule(pl.LightningModule):
    """Lightning module for UNet baseline."""

    def __init__(self, loss: WeightedSumLoss, in_channels=1, lr=1e-3, weight_decay=5e-2, warmup_ratio=0.05):
        super().__init__()
        self.model = get_unet_baseline(in_channels=in_channels)
        self.lr = lr
